...
export const filterAndSortDiemDen = (
  filter?: DiemDen.FilterParams,
  sort?: DiemDen.SortParams,
): DiemDen.Record[] => {
  let result = getAllDiemDen();

  // Lọc theo loại hình
  if (filter?.loaiHinh && filter.loaiHinh !== 'all') {
    result = result.filter((item) => item.loaiHinh === filter.loaiHinh);
  }

  // Lọc theo giá
  if (filter?.giaMin !== undefined) {
    result = result.filter((item) => item.giaVeChung >= filter.giaMin!);
  }
  if (filter?.giaMax !== undefined) {
    result = result.filter((item) => item.giaVeChung <= filter.giaMax!);
  }

  // Lọc theo rating
  if (filter?.ratingMin !== undefined) {
    result = result.filter((item) => item.rating >= filter.ratingMin!);
  }
